...
namespace ByteBank.SistemaAgencia
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var listaSemNull = new List<ContaCorrente>();
            foreach (var conta in contas)
            {
                if(conta != null)
                {
                    listaSemNull.Add(conta);
                }
            }
            var contaNaoNula = contas.Where(conta => conta != null);   

            IOrderedEnumerable<ContaCorrente> contasOrdenadas = listaSemNull.OrderBy(conta => conta.Numero);
